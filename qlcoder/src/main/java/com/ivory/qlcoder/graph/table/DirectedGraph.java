package com.ivory.qlcoder.graph.table;
...
public class DirectedGraph<T> implements GraphInterface<T>, java.io.Serializable {
    ...
    private Map<T, VertexInterface<T>> vertices;// map 对象用来保存图中的所有顶点.T
                                                // 是顶点标识,VertexInterface为顶点对象
    ...
    public void addVertex(T vertexLabel) {
        ...
        vertices.put(vertexLabel, new Vertex(vertexLabel));// new Vertex
                                                           // 对象,会创建一个LinkedList,该LinkedList用来表示该顶点的邻接表
    }
    ...
    public boolean hasEdge(T begin, T end) {
        ...
        while (!found && neighbors.hasNext()) {// 在起始顶点的邻接表中查找 结束顶点endVertex
            ...
            if (endVertex.equals(neighbor))
                found = true;// 若找到了,结束顶点endVertex.
                             // 说明beginVertex和endVertex之间是有边的
        }// end while
        return found;
    }
    ...
    public Queue<T> getDepthFirstTraversal(T origin) {
        ...
        while (!vertexStack.isEmpty()) {
            ...
            if (nextNeighbor != null) {
                ...
                vertexStack.pop();// 当某顶点的所有邻接点都被访问了时,直接将该顶点pop,这样下一次while pop
                                  // 时就回溯到前一个顶点
        }// end while
    ...
    public Queue<T> getBreadthFirstTraversal(T origin) {// origin 标识遍历的初始顶点
        ...
        Queue<T> traversalOrder = new LinkedList<>();// 保存遍历过程中遇到的
                                                     // 顶点标识--整个图的遍历顺序就保存在其中,无出队操作
    ...
    public Stack<T> getTopologicalSort() {
         ...
         * 因为算法导论中介绍的图的数据结构与此处实现的图的数据结构不同 此算法的最坏时间复杂度为O(V*(V+E))==V * max{V,E}
         */
        resetVertices();// 先将所有的顶点初始化
    ...
    public int getChildGraphNumber() {
        ...
        while (true) {
            ...
            for (Map.Entry<T, VertexInterface<T>> entry : vertices.entrySet()) {
                // System.out.println("key:" + entry.getKey() + "; value mark:"
                // + entry.getValue().isMarked());
                if (!entry.getValue().isMarked()) {
                    getBreadthFirstTraversal(entry.getKey());
                    childGraphCount++;
//                    System.out.println("key:" + entry.getKey() + "; value mark:" + entry.getValue().isMarked() + "; count:" + childGraphCount);
                    breakTag = true;
                    break;
                }
            }
...
//            vertexQueue.offer(originVertex);
//            
//            arrayList.remove(originVertex.getLabel());
//
//            while (!vertexQueue.isEmpty()) {
//                VertexInterface<T> frontVertex = vertexQueue.poll();// 出队列,poll()在队列为空时返回null
//                Iterator<VertexInterface<T>> neighbors = frontVertex.getNeighborInterator();
...
//                        vertexQueue.offer(nextNeighbor);// 将该顶点的邻接点入队列
//                        
//                        arrayList.remove(nextNeighbor.getLabel());
//                    }
//                }// end inner while
//            }// end outer while
//        }
