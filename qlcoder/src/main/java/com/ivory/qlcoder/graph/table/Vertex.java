...
class Vertex<T> implements VertexInterface<T>, java.io.Serializable {
    ...
    private boolean marked;// 标志顶点是否加入子图中
    ...
    /**
     * Task: 这里用了一个单独的类来表示边,主要是考虑到带权值的边 可以看出,Edge类封装了一个顶点和一个double类型变量
    ...
    public void visit() {
        this.visited = true;
        this.marked = true;//顶点被访问的时候,同时标记,配合遍历完成子图统计
    }
    ...
    public boolean equals(Object other) {
        ...
        if ((other == null) || (getClass() != other.getClass()))
            ...
            result = label.equals(otherVertex.label);// 节点是否相同最终还是由标识
                                                     // 节点类型的类的equals() 决定
        }
        return result;
    }
